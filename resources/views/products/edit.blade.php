...
<div class="dash-page-header">
    <div>
        ...
        <p class="dash-page-subtitle">Update name, category, description, or unit</p>
    </div>
</div>

@if($categories->isEmpty())
    <div class="dash-card" style="background:var(--dash-brand-10);border-color:var(--dash-brand);">
        <p style="margin:0;font-size:.9rem;color:var(--dash-ink);">Add at least one <a href="{{ route('product-categories.create') }}" class="text-[var(--dash-brand)] font-semibold" wire:navigate>product category</a> before editing products.</p>
    </div>
@else
<div class="dash-card">
    ...
    <form action="{{ route('products.update', $product) }}" method="POST" style="padding:0 20px 20px;">
        @csrf
        @method('PUT')
        <div style="display:flex;flex-direction:column;gap:16px;max-width:400px;">
            ...
            <div>
                ...
                <select id="product_category_id" name="product_category_id" required
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" {{ (string) old('product_category_id', $product->product_category_id) === (string) $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                    @endforeach
                </select>
            ...
            <button type="submit" class="dash-btn dash-btn-brand" style="align-self:flex-start;">Update product</button>
        </div>
    </form>
</div>
@endif
